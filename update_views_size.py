...
new_block = """    # ── BIG TABLE (Row 0 to 9) ──
    ...
    if staff.candidate_photo:
        try:
            # Reduced photo size to match reference
    ...
    # Adjusted column widths to give more space, matching the image strictly
    big_c = [0.6 * inch, 0.4 * inch, 1.9 * inch, 0.9 * inch, 0.9 * inch, 0.8 * inch, 1.0 * inch, 0.9 * inch]
    
    big_data = [
        # 0
        [_p('STAFF ID', 6, True), '', _p(_val(staff.staff_id), 6), _p('Gender', 6, True), _p(gender_display, 6), photo_content, '', ''],
        # 1
        [_p('Full Name', 6, True), '', _p(_val(staff.full_name), 6), _p('Marital Status', 6, True), _p(_val(staff.marital_status), 6), '', '', ''],
        # 2
        [_p('Address', 6, True), _p('Permanent', 5, True), _p(_val(staff.permanent_address), 6), '', '', '', '', ''],
        # 3
        ['', _p('Present', 5, True), _p(_val(staff.present_address), 6), '', '', '', '', ''],
        # 4
        [_p('ID / Passport No.', 6, True), '', _p(_val(staff.id_passport_no), 6), _p('Date of Issue', 6, True), _p(str(staff.date_of_issue) if staff.date_of_issue else '', 6), _p('Issue From', 6, True), _p(_val(staff.issue_from), 6), ''],
        # 5
        [_p('Personal\\nInformation', 6, True, TA_CENTER), '', _p('Date of Birth', 5, True, TA_CENTER), _p('Eye Lense', 5, True, TA_CENTER), '', _p('Blood Group', 5, True, TA_CENTER), _p('Phone No.', 5, True, TA_CENTER), _p('Email ID', 5, True, TA_CENTER)],
        # 6
        ['', '', _p(dob, 6, align=TA_CENTER), _p('Right', 5, True, TA_CENTER), _p(_val(staff.eye_lense_right), 5, align=TA_CENTER), _p(_val(staff.blood_group), 6, align=TA_CENTER), _p(_val(staff.phone_no), 6, align=TA_CENTER), _p(_val(staff.email_id), 5, align=TA_CENTER)],
        # 7
        ['', '', '', _p('Left', 5, True, TA_CENTER), _p(_val(staff.eye_lense_left), 5, align=TA_CENTER), '', '', ''],
        # 8
        [_p('Family Records', 6, True), '', '', '', '', _p('CONTACT NO', 6, True, TA_CENTER), '', ''],
        # 9
        [_p('Spouse Name', 6, True), '', _p(_val(staff.spouse_name), 6), '', '', _p(_val(staff.contact_no), 6), '', ''],
    ]

    # Reduced padding for LESS congestion vertically
    PAD_MIN = 1
    ...
    # ── BANK INFORMATION ──
    # Using Spacer(1, 2) throughout to shrink vertical gaps
    bank_header = [[_p('BANK INFORMATION', 7, True, TA_CENTER)]]
    ...
    for b in bank_qs:
        if b:
            bank_data.append([_p(_val(b.bank_name), 6), _p(_val(b.branch_name), 6), _p(_val(b.account_no), 6, align=TA_CENTER), _p(_val(b.account_holder_name), 6)])
        else:
            bank_data.append(['', '', '', ''])
    ...
    # ── EDUCATIONAL HISTORY ──
    elements.append(_grid([[_p('EDUCATIONAL HISTORY', 7, True, TA_CENTER)]], [W], [('BACKGROUND', (0, 0), (-1, -1), GREY_BG)]))
    
    edu_c = [1.2 * inch, 2.5 * inch, 0.65 * inch, 0.65 * inch, 0.65 * inch, 0.65 * inch, 1.05 * inch]
    edu_all = [
        [_p('Pass Level', 6, True, TA_CENTER), _p('Name of School', 6, True, TA_CENTER), _p('Admission & Graduation', 6, True, TA_CENTER), '', '', '', _p('Enrolled Years', 6, True, TA_CENTER)],
    ...
    # ── WORKING EXPERIENCE ──
    elements.append(_grid([[_p('WORKING EXPERIENCE', 7, True, TA_CENTER)]], [W], [('BACKGROUND', (0, 0), (-1, -1), GREY_BG)]))

    work_all = [
        [_p('Type of Work', 6, True, TA_CENTER), _p('Name of Working Company', 6, True, TA_CENTER), _p('Date of Join & Resign', 6, True, TA_CENTER), '', '', '', _p('Working Years', 6, True, TA_CENTER)],
        ['', '', _p('Year', 6, True, TA_CENTER), _p('Month', 6, True, TA_CENTER), _p('Year', 6, True, TA_CENTER), _p('Month', 6, True, TA_CENTER), '']
    ]
    ...
    ct_data = [
        [_p('CERTIFICATE OF SKILLS', 7, True, TA_CENTER), '', _p('SKILLS TRAINING STATUS', 7, True, TA_CENTER), ''],
        [_p('Pass Year & Month', 5, True, TA_CENTER), _p('Name of Certificate', 5, True, TA_CENTER), _p('Join Year and Month', 5, True, TA_CENTER), _p('Organization', 5, True, TA_CENTER)]
    ]
    ...
    for i in range(max_rows):
        ...
        t = train_qs[i]
        ct_data.append([
            _p(f'{_val(c.pass_year)}/{_val(c.pass_month)}' if c and (c.pass_year or c.pass_month) else '', 5, align=TA_CENTER),
            _p(_val(c.name_of_certificate) if c else '', 5),
            _p(f'{_val(t.join_year)}/{_val(t.join_month)}' if t and (t.join_year or t.join_month) else '', 5, align=TA_CENTER),
            _p(_val(t.organization) if t else '', 5),
        ])
    ...
    dl_data = [
        [_p('DRIVING LICENSE', 6, True, TA_CENTER), _p('Pass Year & Month', 5, True, TA_CENTER), _p('Discretion of License', 5, True, TA_CENTER)],
        ['', _p(f'{_val(license.pass_year)} / {_val(license.pass_month)}' if license else '', 6, align=TA_CENTER), _p(_val(license.discretion_of_license) if license else '', 6)]
    ]
    ...
    # ── HOBBIES + MOTIVATION ──
    hm_data = [
        [_p('Hobbies, Special skills, etc.', 6, True), _p('Motivation, Self-promotion', 6, True)],
        [_p(_val(staff.hobbies), 6), _p(_val(staff.motivation), 6)],
    ]
    hm_tbl = Table(hm_data, colWidths=[W / 2, W / 2], rowHeights=[None, 0.4 * inch])
...
if start_marker in content and end_marker in content:
    ...
    with open('e:/down/AQUA/manager/views.py', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully replaced formatting sizes.")
else:
